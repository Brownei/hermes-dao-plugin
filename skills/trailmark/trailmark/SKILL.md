...
try:
    graph_names = engine.subgroup_names()
except AttributeError:
    graph_names = engine.subgraph_names()
...
**CLI execution issue**: `npx trailmark` often fails. Use the Python API via `uv run python3 -` (stdin mode) with an inline heredoc script instead.

**Full graph analysis**: For large codebases (DAO-DAO-UI has 79k+ objects), the full graph via Python API is more reliable than the CLI `analyze --summary` command.

**API method naming**: Some Trailmark versions use `subgroup_names()` while others use `subgraph_names()` — wrap in try/except as shown above.

**Installation fallback**: If `uv run trailmark` fails with ModuleNotFoundError, try `uv pip install trailmark` instead of assuming the project is pre-configured.

**Graph JSON double-escaping**: The pre-generated `trailmark-graph.json` is double-escaped. Before loading, decode with `content.encode('utf-8').decode('unicode_escape')` then `json.loads()`.

**QMD + Trailmark combination**: For semantic search, combine Trailmark's pre-built graph (queryable via Python API for code semantics) with QMD for natural language queries. Use `qmd query "keywords"` for BM25 (works without embeddings) plus Trailmark's node graph for AST-level search.

**Telemetry config**: When running Trailmark via CLI for performance-sensitive analysis, configure telemetry to minimize overhead:
```bash
uv run trailmark analyze --language auto --context-window-size 4096 --telemetry=false {targetDir}
```
