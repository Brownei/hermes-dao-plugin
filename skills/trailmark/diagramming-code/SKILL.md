...
or if `diagram.py` is missing, use the **fallback API pattern** documented
in the Pitfalls section above. This is preferable to manually parsing code.

---

## Quick Start

**Primary method - Using diagram.py script:**
```bash
# Set baseDir to the directory containing the diagram.py script
# Usually the trailmark installation directory or a local fork
export baseDir=/path/to/trailmark

# Find the script location if you don't know it:
find ~/.local/share/uv -name "diagram.py" -path "*/trailmark*" 2>/dev/null
# OR if installed globally: find /usr -name "diagram.py" -path "*/trailmark*" 2>/dev/null
...
**Fallback - Direct programmatic API when script is missing:**
When `diagram.py` isn't available, use Trailmark's Python API directly.
See `scripts/fallback-diagram.py` template for generating diagrams via QueryEngine.
...
```bash
# If script doesn't exist, use the API directly:
uv run python3 scripts/fallback-diagram.py --target {targetDir} --type call-graph --focus CwInfuser --depth 2
```
...
### diagram.py missing or baseDir not set
The `{baseDir}` template variable **must** be resolved before use. It points to the directory containing `scripts/diagram.py`. Common locations:
- `/opt/homebrew/lib/python3.12/site-packages/trailmark/` (brew install)
- `~/.local/share/uv/data/trailmark/` (uv install)
- Local checkout of trailmark repository

If the script is not found, use the **fallback API method** below instead of handing off the task.

### Fallback API pattern (when diagram.py is missing)
```python
from trailmark.query.api import QueryEngine
import json
...
# Get nodes matching a query (e.g., Contract calls)
callers = engine.callers_of("CwInfuser")  # returns list of CallEdge
callees = engine.callees_of("CwInfuser")  # returns list of CallEdge

# Generate Mermaid manually (see references/sanitize.py for ID sanitization utilities)
nodes = set()
edges = []

for caller in callers:
    nodes.add(caller.source)
    nodes.add(caller.target)
    edges.append((caller.source, caller.target, caller.confidence))

# Build flowchart string...
```
