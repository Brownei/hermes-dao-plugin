...
def boards_root() -> Path:
    """Return ``<root>/kanban/boards`` — the parent of all board directories.

    Every board lives at ``<root>/kanban/boards/<slug>/`` with its own
    ``kanban.db``, ``board.json``, ``workspaces/``, and ``logs/``.
    """
...
def get_current_board() -> Optional[str]:
    ...
    Returns ``None`` when no board is selected or no boards exist.
    """
...
def clear_current_board() -> None:
    """Remove ``<root>/kanban/current`` so no board is selected."""
    try:
...
def board_dir(board: Optional[str] = None) -> Path:
    ...
    Raises :class:`ValueError` if no board is specified.
    All boards live at ``<root>/kanban/boards/<slug>/`` with
    everything inside that directory including the ``kanban.db``.
    """
    slug = _normalize_board_slug(board)
    if slug is None:
        raise ValueError("board slug is required — no board is currently selected")
...
def board_exists(board: Optional[str] = None) -> bool:
    ...
    Returns False if no board slug is provided.
    """
    slug = _normalize_board_slug(board)
    if slug is None:
        return False
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    3. Raises :class:`ValueError` if no board can be resolved.

    All boards use ``<root>/kanban/boards/<slug>/kanban.db``.
    """
    ...
    if slug is None:
        raise ValueError(
            "No board specified and no board is currently selected. "
            "Use --board <slug> or create a board first."
        )
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    Raises :class:`ValueError` if no board can be resolved.
    """
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    Logs follow the board — makes ``hermes kanban log`` unambiguous even
    when multiple boards have tasks with the same id.
...
def board_metadata_path(board: Optional[str] = None) -> Path:
    ...
    Raises :class:`ValueError` if no board slug is provided.
    """
    slug = _normalize_board_slug(board)
    if slug is None:
        raise ValueError("board slug is required")
...
def create_board(
    ...
    *,
    ...
    # Create default columns if not already present
    _ensure_default_columns(normed, columns)
    # Insert creator as admin if wallet provided
...
def _ensure_default_columns(board_slug: str, custom_columns: Optional[list[dict]] = None):
    """Create default columns for a new board if none exist yet."""
    conn = connect(board=board_slug)
    try:
        existing = conn.execute(
            "SELECT COUNT(*) as cnt FROM board_columns WHERE board_slug = ?",
            (board_slug,),
        ).fetchone()
        if existing and existing["cnt"] > 0:
            return  # columns already exist

        cols = custom_columns if custom_columns else _DEFAULT_COLUMNS
        now = int(time.time())
        for i, col in enumerate(cols):
            conn.execute(
                "INSERT INTO board_columns (board_slug, name, color, sort_order, created_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (board_slug, col["name"], col.get("color", ""), i, now),
            )
    finally:
        conn.close()
...
def list_boards(*, include_archived: bool = True) -> list[dict]:
    ...
    Scans ``boards/`` for subdirectories that either contain a
    ``kanban.db`` or a ``board.json``.

    Returns a list of metadata dicts, sorted alphabetically.
...
def remove_board(slug: str, *, archive: bool = True) -> dict:
    ...
    # If the user removed the currently-active board, clear the selection.
...
# ---------------------------------------------------------------------------
# Board members (wallet-based access control)
# ---------------------------------------------------------------------------
...
def remove_board_member(
    ...
    """Remove a wallet address from a board's membership.

    Returns True if a row was actually deleted.
    """
    with write_txn(conn):
        cur = conn.execute(
            "DELETE FROM board_members WHERE board_slug = ? AND wallet_addr = ?",
            (board_slug, wallet_addr),
        )
    return cur.rowcount > 0


def get_board_members(conn: sqlite3.Connection, board_slug: str) -> list[dict]:
    """Return all members of a board, ordered by role then joined_at."""
    rows = conn.execute(
        "SELECT wallet_addr, role, joined_at "
        "FROM board_members WHERE board_slug = ? "
        "ORDER BY CASE role WHEN 'admin' THEN 1 WHEN 'editor' THEN 2 ELSE 3 END, joined_at",
        (board_slug,),
    ).fetchall()
...
def get_member_role(
    conn: sqlite3.Connection, board_slug: str, wallet_addr: str
) -> Optional[str]:
    """Return the role of a wallet address on a board, or None if not a member."""
    row = conn.execute(
        "SELECT role FROM board_members WHERE board_slug = ? AND wallet_addr = ?",
        (board_slug, wallet_addr),
    ).fetchone()
...
def check_board_permission(
    ...
    """Check if a wallet address has at least the required role on a board.
    ...
    if not row:
        return False
    user_level = role_hierarchy.get(row["role"], -1)
...
def get_boards_for_wallet(wallet_addr: str) -> list[dict]:
    """Return all boards that a wallet address is a member of.

    Each entry includes the board metadata and the user's role.
    """
    ...
    for child in sorted(boards_dir.iterdir(), key=lambda p: p.name.lower()):
        ...
        conn = connect(board=normed)
        try:
            role = get_member_role(conn, normed, wallet_addr)
            if role is None:
                continue
            meta = read_board_metadata(normed)
            meta["my_role"] = role
...
class Share:
    """Represents a board share link."""
    __slots__ = ("id", "board_slug", "owner_address", "role",
                 "expires_at", "created_at", "last_used_at",
                 "use_count", "is_revoked")

    def __init__(self, id, board_slug, owner_address, role, expires_at,
                 created_at, last_used_at=None, use_count=0, is_revoked=0):
        self.id = id
...
def get_board_columns(board_slug: str) -> list[dict]:
    """Return all columns for a board, ordered by sort_order."""
    conn = connect(board=board_slug)
    try:
        rows = conn.execute(
            "SELECT id, name, color, sort_order, created_at "
            "FROM board_columns WHERE board_slug = ? ORDER BY sort_order",
            (board_slug,),
        ).fetchall()
        return [dict(r) for r in rows]
    finally:
...
def add_board_column(board_slug: str, name: str, color: str = "") -> dict:
    """Add a new column to a board. Raises ValueError if name already exists."""
    conn = connect(board=board_slug)
    try:
        max_order = conn.execute(
            "SELECT COALESCE(MAX(sort_order), -1) as mx FROM board_columns WHERE board_slug = ?",
            (board_slug,),
        ).fetchone()
        ...
        try:
            conn.execute(
                "INSERT INTO board_columns (board_slug, name, color, sort_order, created_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (board_slug, name, color, sort_order, now),
            )
        except sqlite3.IntegrityError:
            raise ValueError(f"Column {name!r} already exists on board {board_slug!r}")
        row = conn.execute(
            "SELECT id, name, color, sort_order, created_at FROM board_columns WHERE board_slug = ? AND name = ?",
            (board_slug, name),
        ).fetchone()
...
def update_board_column(board_slug: str, col_id: int, name: Optional[str] = None, color: Optional[str] = None) -> Optional[dict]:
    """Update a column's name and/or color."""
    conn = connect(board=board_slug)
    try:
        sets, vals = [], []
        if name is not None:
        ...
        if color is not None:
            ...
            vals.append(color)
        if not sets:
            return get_board_column_by_id(board_slug, col_id)
        vals.append(col_id)
        cur = conn.execute(
            f"UPDATE board_columns SET {', '.join(sets)} WHERE board_slug = ? AND id = ?",
            vals,
        )
        if cur.rowcount == 0:
            return None
...
def delete_board_column(board_slug: str, col_id: int) -> bool:
    """Delete a column. Tasks in that column move to the first column."""
    conn = connect(board=board_slug)
    try:
        col = conn.execute(
            "SELECT name FROM board_columns WHERE board_slug = ? AND id = ?",
            (board_slug, col_id),
        ).fetchone()
        if not col:
            return False
        old_name = col["name"]
        # Find the first column to move tasks to
        first = conn.execute(
            "SELECT name FROM board_columns WHERE board_slug = ? AND id != ? ORDER BY sort_order LIMIT 1",
            (board_slug, col_id),
        ).fetchone()
        new_name = first["name"] if first else None
        # Move tasks
        if new_name and new_name in VALID_STATUSES:
            conn.execute(
                "UPDATE tasks SET status = ? WHERE board_slug IS NULL AND status = ?",
                (new_name, old_name),
            )
        # Delete the column
        cur = conn.execute(
            "DELETE FROM board_columns WHERE board_slug = ? AND id = ?",
            (board_slug, col_id),
        )
        return cur.rowcount > 0
    finally:
...
def reorder_board_columns(board_slug: str, col_ids: list[int]) -> bool:
    """Reorder columns by setting sort_order based on list position."""
    conn = connect(board=board_slug)
    try:
        for i, col_id in enumerate(col_ids):
            conn.execute(
                "UPDATE board_columns SET sort_order = ? WHERE board_slug = ? AND id = ?",
                (i, board_slug, col_id),
            )
        return True
    finally:
...
def get_board_column_by_id(board_slug: str, col_id: int) -> Optional[dict]:
    """Get a single column by ID."""
    conn = connect(board=board_slug)
    try:
        row = conn.execute(
            "SELECT id, name, color, sort_order, created_at "
            "FROM board_columns WHERE board_slug = ? AND id = ?",
            (board_slug, col_id),
        ).fetchone()
...
-- Custom board columns. Each board can define its own columns; tasks
-- reference them by name. The default set (Todo, In Progress, Done) is
-- created automatically when a board is created.
...
CREATE INDEX IF NOT EXISTS idx_board_columns_slug    ON board_columns(board_slug, sort_order);
...
CREATE INDEX IF NOT EXISTS idx_board_members_addr ON board_members(wallet_addr);

-- JWT-based share links. The share_id is a signed JWT token.
