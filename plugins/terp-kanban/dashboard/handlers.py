...
def _resolve_board(board: Optional[str]) -> Optional[str]:
    ...
    if normed and not kanban_db.board_exists(normed):
        raise HTTPException(
            status_code=404,
...
def specify_task_endpoint(
    ...
    try:
        os.environ["HERMES_KANBAN_BOARD"] = board or ""
...
# ===========================================================================
# Wallet authentication (Cosmos — Keplr / Leap / Cosmostation)
# ===========================================================================
...
try:
    ...
    from ecdsa import VerifyingKey as _VerifyingKey, SECP256k1 as _SECP256k1
    from ecdsa.util import sigdecode_string as _sigdecode_string
    _WALLET_CRYPTO_AVAILABLE = True
except ImportError:
    _WALLET_CRYPTO_AVAILABLE = False

# {nonce: {"chain_id": str, "expires_at": int}}
_WALLET_CHALLENGES: dict[str, dict] = {}

# {session_token: {"wallet_addr": str, "chain_id": str, "expires_at": int}}
_WALLET_SESSIONS: dict[str, dict] = {}
...
def _get_chains() -> list[dict]:
    return _get_wallet_config().get("chains", [])


def _get_chain(chain_id: str) -> Optional[dict]:
...
def _get_session_ttl() -> int:
    return _get_wallet_config().get("session_ttl_seconds", 86400)


def _get_challenge_ttl() -> int:
    return _get_wallet_config().get("challenge_ttl_seconds", 300)


def _decode_bech32(addr: str) -> tuple[str, bytes]:
    prefix, data = _bech32.bech32_decode(addr)
    if prefix is None or data is None:
        raise ValueError(f"Invalid Bech32 address: {addr}")
    data_bytes = bytes(_bech32.convertbits(data, 5, 8, False))
    return prefix, data_bytes


def _encode_bech32(prefix: str, data: bytes) -> str:
    bits = _bech32.convertbits(data, 8, 5)
    if bits is None:
        raise ValueError("Failed to convert bits")
    return _bech32.bech32_encode(prefix, bits)


def _pubkey_to_address(pubkey_bytes: bytes, bech32_prefix: str) -> str:
    sha256_hash = _hashlib.sha256(pubkey_bytes).digest()
    addr_data = sha256_hash[:20]
    return _encode_bech32(bech32_prefix, addr_data)
...
def _verify_signature(
    ...
    try:
        ...
        pubkey = _b64.b64decode(pubkey_b64)
        msg_hash = _hashlib.sha256(message).digest()
        vk = _VerifyingKey.from_string(pubkey, curve=_SECP256k1)
        vk.verify(sig, msg_hash, sigdecode=_sigdecode_string)
        return True
    except Exception:
        return False


def _get_wallet_from_request(request: Request) -> Optional[dict]:
    session_token = request.headers.get("x-wallet-session")
    if not session_token:
        return None
    session = _WALLET_SESSIONS.get(session_token)
    if not session:
        return None
    if session["expires_at"] < int(time.time()):
        _WALLET_SESSIONS.pop(session_token, None)
        return None
    return session


def _require_wallet(request: Request) -> dict:
    wallet = _get_wallet_from_request(request)
    if not wallet:
        raise HTTPException(
            status_code=401,
            detail="Wallet authentication required. Connect your wallet first.",
        )
    return wallet


def _require_board_member(
    request: Request, board_slug: str, required_role: str = "viewer"
) -> dict:
    wallet = _require_wallet(request)
    conn = kanban_db.connect(board=board_slug)
    try:
        has_perm = kanban_db.check_board_permission(
            conn, board_slug, wallet["wallet_addr"], required_role
        )
        if not has_perm:
            raise HTTPException(
                status_code=403,
                detail=f"You do not have {required_role} access to this board.",
            )
        return wallet
    finally:
        conn.close()
...
def _require_board_admin(request: Request, board_slug: str) -> dict:
    return _require_board_member(request, board_slug, required_role="admin")
...
class WalletCreateBoardRequest(BaseModel):
    slug: str
    name: str
    description: str = ""
    icon: str = ""
...
class WalletAddMemberRequest(BaseModel):
    wallet_addr: str
    role: str


class WalletUpdateMemberRoleRequest(BaseModel):
    role: str


@router.get("/wallet/config")
def get_wallet_config():
    wallet_cfg = _get_wallet_config()
    return {
        "enabled": wallet_cfg.get("enabled", True),
        "required": wallet_cfg.get("required", True),
        "chains": _get_chains(),
...
def get_wallet_challenge(chain_id: str = Query(...)):
    ...
    expires_at = int(time.time()) + _get_challenge_ttl()
    _WALLET_CHALLENGES[nonce] = {"chain_id": chain_id, "expires_at": expires_at}
    now = int(time.time())
    expired = [n for n, c in _WALLET_CHALLENGES.items() if c["expires_at"] < now]
    for n in expired:
        _WALLET_CHALLENGES.pop(n, None)
    return {
        "nonce": nonce,
        "chain_id": chain_id,
...
def verify_wallet(payload: WalletVerifyRequest):
    if not _WALLET_CRYPTO_AVAILABLE:
        raise HTTPException(
            status_code=500,
            detail="Wallet crypto not available. Install: pip install bech32 ecdsa",
        )
    chain = _get_chain(payload.chain_id)
    if not chain:
        raise HTTPException(status_code=400, detail=f"Unknown chain: {payload.chain_id}")
    challenge = _WALLET_CHALLENGES.get(payload.challenge_nonce)
    if not challenge:
        raise HTTPException(status_code=400, detail="Expired or invalid challenge nonce")
    if challenge["chain_id"] != payload.chain_id:
        raise HTTPException(status_code=400, detail="Chain ID mismatch challenge")
    if challenge["expires_at"] < int(time.time()):
        _WALLET_CHALLENGES.pop(payload.challenge_nonce, None)
        raise HTTPException(status_code=400, detail="Challenge has expired")
    try:
        prefix, _addr_data = _decode_bech32(payload.address)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid Bech32 address")
    expected_prefix = chain.get("bech32_prefix", "")
    if prefix != expected_prefix:
        raise HTTPException(
            status_code=400,
            detail=f"Address prefix mismatch: expected {expected_prefix}, got {prefix}",
        )
    try:
        pubkey_bytes = _b64.b64decode(payload.pubkey)
        derived_addr = _pubkey_to_address(pubkey_bytes, expected_prefix)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid pubkey")
    if derived_addr != payload.address:
        raise HTTPException(
            status_code=401,
            detail="Address does not match pubkey. Possible spoofing attempt.",
        )
    message = f"terp-kanban:{payload.challenge_nonce}".encode()
    if not _verify_signature(message, payload.signature, payload.pubkey):
        raise HTTPException(status_code=401, detail="Signature verification failed")
    _WALLET_CHALLENGES.pop(payload.challenge_nonce, None)
    session_token = _secrets.token_urlsafe(48)
    expires_at = int(time.time()) + _get_session_ttl()
    _WALLET_SESSIONS[session_token] = {
        "wallet_addr": payload.address,
        "chain_id": payload.chain_id,
        "expires_at": expires_at,
    }
    now = int(time.time())
    expired = [t for t, s in _WALLET_SESSIONS.items() if s["expires_at"] < now]
    for t in expired:
        _WALLET_SESSIONS.pop(t, None)
    return {
...
def logout_wallet(request: Request):
    session_token = request.headers.get("x-wallet-session")
    if session_token:
        _WALLET_SESSIONS.pop(session_token, None)
    return {"ok": True}
...
def get_wallet_me(request: Request):
    wallet = _require_wallet(request)
    return wallet


@router.post("/wallet/boards")
def wallet_create_board(payload: WalletCreateBoardRequest, request: Request):
    wallet = _require_wallet(request)
    try:
        meta = kanban_db.create_board(
            slug=payload.slug,
            ...
            icon=payload.icon,
            color=payload.color,
            created_by_wallet=wallet["wallet_addr"],
        )
...
@router.get("/wallet/boards/public")
def public_list_boards():
    """Return all boards without requiring wallet authentication.
    Used when wallet is optional and the user is not connected."""
...
def wallet_list_boards(request: Request):
    """Return ALL boards. Connected wallet users default to 'viewer' role
    if they aren't explicitly listed as a member. If a board has NO members
    at all, the first visitor becomes admin."""
    wallet = _require_wallet(request)
    ...
    if boards_dir.is_dir():
        for child in sorted(boards_dir.iterdir(), key=lambda p: p.name.lower()):
            ...
            try:
                role = kanban_db.get_member_role(conn, normed, wallet_addr)
                if role is None:
                    # Check if board has any members at all
                    all_members = kanban_db.get_board_members(conn, normed)
                    if not all_members:
                        # First visitor becomes admin
                        kanban_db.add_board_member(conn, normed, wallet_addr, role="admin")
                        role = "admin"
                    else:
                        # Auto-add as viewer on first visit
                        kanban_db.add_board_member(conn, normed, wallet_addr, role="viewer")
                        role = "viewer"
                meta = kanban_db.read_board_metadata(normed)
                meta["my_role"] = role
                boards.append(meta)
...
def wallet_get_board(board_slug: str, request: Request):
    """Get board details. Any connected wallet can view; auto-added as viewer.
    If board has no members, first visitor becomes admin."""
    wallet = _require_wallet(request)
    wallet_addr = wallet["wallet_addr"]
    ...
    conn = kanban_db.connect(board=board_slug)
    try:
        role = kanban_db.get_member_role(conn, board_slug, wallet_addr)
        if role is None:
            all_members = kanban_db.get_board_members(conn, board_slug)
            if not all_members:
                kanban_db.add_board_member(conn, board_slug, wallet_addr, role="admin")
                role = "admin"
            else:
                kanban_db.add_board_member(conn, board_slug, wallet_addr, role="viewer")
                role = "viewer"
        members = kanban_db.get_board_members(conn, board_slug)
        columns = kanban_db.get_board_columns(board_slug)
    finally:
        conn.close()
    return {
        "board": meta,
...
def wallet_delete_board(board_slug: str, request: Request):
    _require_board_admin(request, board_slug)
    if not kanban_db.board_exists(board_slug):
        raise HTTPException(status_code=404, detail=f"Board {board_slug!r} not found")
    result = kanban_db.remove_board(board_slug, archive=False)
    return {"ok": True, "result": result}


@router.get("/wallet/boards/{board_slug}/members")
def wallet_list_members(board_slug: str, request: Request):
    """List members. Any connected wallet can view; auto-added as viewer.
    If board has no members, first visitor becomes admin."""
    wallet = _require_wallet(request)
    wallet_addr = wallet["wallet_addr"]
...
def wallet_add_member(board_slug: str, payload: WalletAddMemberRequest, request: Request):
    _require_board_admin(request, board_slug)
    if payload.role not in ("editor", "viewer"):
        raise HTTPException(
            status_code=400,
            detail="Admins can only add members as 'editor' or 'viewer'.",
        )
    try:
        _decode_bech32(payload.wallet_addr)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid Bech32 address")
    if not kanban_db.board_exists(board_slug):
        raise HTTPException(status_code=404, detail=f"Board {board_slug!r} not found")
    conn = kanban_db.connect(board=board_slug)
    try:
        ok = kanban_db.add_board_member(
            conn, board_slug, payload.wallet_addr, role=payload.role
        )
        if not ok:
            raise HTTPException(status_code=400, detail="Failed to add member")
    finally:
        conn.close()
    return {"ok": True, "wallet_addr": payload.wallet_addr, "role": payload.role}


@router.delete("/wallet/boards/{board_slug}/members/{wallet_addr:path}")
def wallet_remove_member(board_slug: str, wallet_addr: str, request: Request):
    _require_board_admin(request, board_slug)
    if not kanban_db.board_exists(board_slug):
        raise HTTPException(status_code=404, detail=f"Board {board_slug!r} not found")
    conn = kanban_db.connect(board=board_slug)
    try:
        target_role = kanban_db.get_member_role(conn, board_slug, wallet_addr)
        if target_role is None:
            raise HTTPException(status_code=404, detail=f"Member {wallet_addr!r} not found")
        if target_role == "admin":
            raise HTTPException(
                status_code=403,
                detail="Cannot remove other admins. Only editors and viewers can be removed.",
            )
        ok = kanban_db.remove_board_member(conn, board_slug, wallet_addr)
    finally:
        conn.close()
    return {"ok": ok, "wallet_addr": wallet_addr}


@router.patch("/wallet/boards/{board_slug}/members/{wallet_addr:path}/role")
def wallet_update_member_role(
    board_slug: str,
    wallet_addr: str,
    payload: WalletUpdateMemberRoleRequest,
    request: Request,
):
    _require_board_admin(request, board_slug)
    if payload.role not in ("editor", "viewer"):
        raise HTTPException(
            status_code=400,
            detail="Can only set role to 'editor' or 'viewer'.",
        )
    if not kanban_db.board_exists(board_slug):
        raise HTTPException(status_code=404, detail=f"Board {board_slug!r} not found")
    conn = kanban_db.connect(board=board_slug)
    try:
        current_role = kanban_db.get_member_role(conn, board_slug, wallet_addr)
        if current_role is None:
            raise HTTPException(status_code=404, detail=f"Member {wallet_addr!r} not found")
        if current_role == "admin" and wallet_addr != _require_wallet(request)["wallet_addr"]:
            raise HTTPException(
                status_code=403,
                detail="Cannot change role of another admin.",
            )
        ok = kanban_db.add_board_member(conn, board_slug, wallet_addr, role=payload.role)
    finally:
        conn.close()
    return {"ok": ok, "wallet_addr": wallet_addr, "role": payload.role}
...
class CreateShareRequest(BaseModel):
    board: str
    role: str = "viewer"
    expires_in_days: Optional[str] = None


@router.post("/shares")
def create_share(payload: CreateShareRequest, request: Request, board: Optional[str] = Query(None)):
    """Create a share link for a board. Admin or editor only."""
    wallet = _require_wallet(request)
    wallet_addr = wallet["wallet_addr"]

    board_slug = _resolve_board(payload.board or board)
    if not board_slug:
        raise HTTPException(status_code=400, detail="board is required")
    if not kanban_db.board_exists(board_slug):
        raise HTTPException(status_code=404, detail=f"Board {board_slug!r} not found")

    # Check user has at least editor role on the board
    conn = kanban_db.connect(board=board_slug)
    try:
        role = kanban_db.get_member_role(conn, board_slug, wallet_addr)
        if role is None:
        ...
        if role not in ("admin", "editor"):
            raise HTTPException(
                status_code=403,
                detail="Only admins and editors can create share links.",
            )
    finally:
        conn.close()
    ...
    # Parse expiration
    expires_in = payload.expires_in_days
    ...
    # Create JWT token
    try:
        jwt_mod = _importlib.import_module("plugins.kanban.dashboard.jwt_sharing")
    except Exception:
        jwt_mod = None
    if jwt_mod is None:
        try:
            from . import jwt_sharing as jwt_mod
        except Exception:
            jwt_mod = None

    if jwt_mod:
        share_id = jwt_mod.create_share_token(
        ...
        share_id = _secrets.token_urlsafe(32)

    share = kanban_db.create_board_share(
        board_slug=board_slug,
        owner_address=wallet_addr,
        role=payload.role,
        expires_at=expires_at,
...
@router.get("/shares")
def list_shares(board: str = Query(..., description="Board slug"), request: Request = None):
    ...
    shares = kanban_db.list_board_shares(board_slug)
    return {
        "shares": [
            {
                "share_id": s.id,
...
def revoke_share(share_id: str, request: Request):
    """Revoke a share link."""
    _require_wallet(request)
    success = kanban_db.revoke_board_share(share_id)
    if not success:
...
def view_shared_board(share_token: str):
    ...
    if jwt_mod:
        payload = jwt_mod.validate_share_token(share_token)
    else:
        payload = None
    ...
    board_slug = payload.get("sub") or payload.get("board")
    role = payload.get("role", "viewer")

    share = kanban_db.get_board_share(share_token)
    if not share:
        return {
            "error": "not_found",
...
def get_shared_board_data(share_token: str, board: Optional[str] = Query(None)):
    """Get the full board data for a shared view."""
    share = kanban_db.validate_share_token(share_token)
    if not share:
...
def validate_share(share_id: str):
    """Validate a share token and return board access info."""
    try:
        jwt_mod = _importlib.import_module("plugins.kanban.dashboard.jwt_sharing")
    except Exception:
    ...
    if jwt_mod:
        payload = jwt_mod.validate_share_token(share_id)
    else:
        payload = None
