...
(function () {
  ...
  function KanbanPage() {
    ...
    // Share state for JWT-based sharing
    const [showShareDialog, setShowShareDialog] = useState(false);
    const [shareData, setShareData] = useState(null);     // {token, board, role} for shared view
    ...
    // --- check for share URL (JWT token in path) --------------------------------
    const checkShareUrl = useCallback(function () {
      ...
      try {
        ...
        const valid = await SDK.fetchJSON(`${API}/share/${encodeURIComponent(token)}`);
        if (valid.error) {
          setShareError(valid.message || valid.error);
          return false;
        }

        // Then fetch the actual board data
        const data = await SDK.fetchJSON(`${API}/share/${encodeURIComponent(token)}/data`);

        setShareInfo({
          token: token,
          board: valid.board_slug,
          role: valid.role,
          readOnly: valid.role === "viewer",
          owner: valid.owner,
          expiresAt: valid.expires_at,
        });

        // Load the board data directly (bypass normal fetch)
        setBoardData({
          board: data.board,
        ...
        setBoard(data.board.slug);

        return true;
      } catch (err) {
        setShareError("Failed to load shared board: " + String(err));
        return false;
      }
    }, []);

    // Check share URL on mount
    useEffect(function () {
      checkShareUrl();
    }, [checkShareUrl]);
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "hermes-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          ...
          onShareClick: function () { setShowShareDialog(true); },
          readOnly: shareData ? shareData.readOnly : false,
        }),
        ...
        // Share dialog
        showShareDialog ? h(ShareDialog, {
          board: board,
          onCancel: function () { setShowShareDialog(false); },
          onShareCreated: function (shareInfo) {
            setShowShareDialog(false);
            // Show share info (link) - could store in state for display
        ...
        selectedTaskId ? h(TaskDrawer, {
          ...
          readOnly: shareData ? shareData.readOnly : false,
          shareRole: shareData ? shareData.role : null,
        }) : null,
      ),
    );
  }
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "unblock") {
        ...
        SDK.fetchJSON(url, {
          ...
          setMsg({
            ok: true, text: tx(t, "unblockedMessage",
      ...
      if (action.kind === "reclaim") {
        ...
        SDK.fetchJSON(url, {
          ...
          setMsg({
            ok: true, text: tx(t, "reclaimedMessage",
  ...
  function BoardSwitcher(props) {
    ...
    return h("div", { className: "hermes-kanban-boardswitcher" },
      h("div", { className: "hermes-kanban-boardswitcher-inner" },
        ...
        // Share button - only show if not in read-only mode
        !props.readOnly && h(Button, {
          onClick: props.onShareClick,
          size: "sm",
          variant: "outline",
          className: "h-8",
          title: "Share this board with a JWT link",
        }, tx(t, "share", "Share")),
        props.readOnly && h("span", {
          className: "text-xs text-muted-foreground px-2",
          title: "View-only mode"
        }, "👁 " + tx(t, "readOnly", "Read-only")),
        props.board !== "default"
          ? h(Button, {
            onClick: function () {
  ...
  // -------------------------------------------------------------------------
  // Share Dialog - JWT-based board sharing
  // -------------------------------------------------------------------------
  ...
  function ShareDialog(props) {
    ...
    function onSubmit(ev) {
      if (ev) ev.preventDefault();
      if (!ownerAddress.trim()) { setErr("Address is required"); return; }
      setSubmitting(true);
      setErr(null);

      const expiresIn = days === "never" ? null : parseInt(days, 10);

      SDK.fetchJSON(withBoard(`${API}/shares`, props.board), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
        ...
        .then(function (data) {
          ...
          setShareLink(baseUrl + data.share_id);
          setSubmitting(false);
        })
        .catch(function (e) {
          setErr(String(e && e.message ? e.message : e));
    ...
    return h("div", {
      ...
      h("form", {
        ...
        h("div", { className: "hermes-kanban-dialog-title" },
          tx(t, "shareBoard", "Share board")),
        h("div", { className: "text-xs text-muted-foreground mb-2" },
          tx(t, "shareDescription",
            "Share this board with a bech32 address. Viewers can comment but not modify tasks.")),

        shareLink ? h("div", { className: "flex flex-col gap-3" },
          h("div", { className: "text-xs text-muted-foreground" },
            tx(t, "shareLink", "Share link:")),
          h("div", { className: "flex gap-2" },
            h("input", {
              type: "text",
              value: shareLink,
              readOnly: true,
              className: "flex-1 h-8 px-2 text-xs bg-muted border rounded font-mono",
            }),
            h(Button, {
              type: "button",
              onClick: copyLink,
          ...
          h("div", { className: "flex gap-2 mt-2" },
            h(Button, {
              type: "button",
              onClick: function () { setShareLink(null); setOwnerAddress(""); },
              size: "sm",
              variant: "outline",
            }, tx(t, "createAnother", "Create another")),
            h(Button, {
              type: "button",
              onClick: props.onCancel,
              size: "sm",
          ...
          h("div", { className: "flex flex-col gap-1" },
            h(Label, { className: "text-xs" }, tx(t, "shareWith", "Share with address"), " ",
              h("span", { className: "text-muted-foreground" },
                tx(t, "terpAddress", "(terp1...)"))),
            h(Input, {
              value: ownerAddress,
              onChange: function (e) { setOwnerAddress(e.target.value); },
              placeholder: "terp1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq5ffndw",
              autoFocus: true,
              className: "h-8 font-mono text-xs",
            }),
          ),
          h("div", { className: "flex flex-col gap-1" },
            h(Label, { className: "text-xs" }, tx(t, "role", "Role")),
            h("div", { className: "flex gap-4" },
              h("label", { className: "flex items-center gap-2 text-sm" },
                h("input", {
                  ...
                  value: "viewer",
                  checked: role === "viewer",
                  onChange: function () { setRole("viewer"); },
                }),
                tx(t, "viewer", "Viewer (comment only)"),
              ),
              h("label", { className: "flex items-center gap-2 text-sm" },
                h("input", {
                  type: "radio",
                  ...
                  value: "editor",
                  checked: role === "editor",
                  onChange: function () { setRole("editor"); },
                }),
                tx(t, "editor", "Editor (full access)"),
              ),
            ),
          ),
          h("div", { className: "flex flex-col gap-1" },
            h(Label, { className: "text-xs" }, tx(t, "expiresIn", "Expires in")),
            h("select", {
              value: days,
        ...
        !shareLink && h("div", { className: "hermes-kanban-dialog-actions" },
          h(Button, {
            type: "button",
            onClick: props.onCancel,
            size: "sm",
          ...
          h(Button, {
            type: "submit",
            size: "sm",
            disabled: submitting || !ownerAddress.trim(),
          }, submitting ? tx(t, "creating", "Creating…") : tx(t, "createShare", "Create link")),
        ),
      ),
    );
  }
  ...
  function TaskCard(props) {
    ...
    return h("div", {
      ...
      className: cn(
        ...
        props.readOnly ? "hermes-kanban-card--readonly" : "",
        stalenessClass(t),
      ),
      ...
      h(Card, null,
        h(CardContent, { className: "hermes-kanban-card-content" },
          h("div", { className: "hermes-kanban-card-row" },
            ...
            t.tenant
              ? h(Badge, {
                variant: "outline", className: "hermes-kanban-tag",
  ...
  function TaskDrawer(props) {
    ...
    const handleComment = function () {
      // Allow comments if not readOnly OR if shareRole is viewer
      if (props.readOnly && props.shareRole !== "viewer") return;
    ...
    const doPatch = function (patch, opts) {
      ...
      if (props.readOnly) {
        setErr("Read-only mode: cannot modify tasks");
    ...
    return h("div", { className: "hermes-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        ...
        // Show comment input if: not readOnly OR (readOnly AND viewer can comment)
        data ? h("div", { className: "hermes-kanban-drawer-comment-row" },
          (function () {
            // Allow comments if not readOnly OR if shareRole is viewer
            const canComment = !props.readOnly || props.shareRole === "viewer";
            if (!canComment) {
              return h("span", { className: "text-xs text-muted-foreground" },
                tx(t, "commentsDisabled", "Comments disabled in read-only mode"));
            }
            return [
              h(Input, {
                key: "input",
